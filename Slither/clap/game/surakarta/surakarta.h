...
constexpr int kPolicyDim = kNumOfGrids * kNumOfGrids;
...
class SurakartaState final : public State {
 public:
  SurakartaState(GamePtr);
 ...
 private:
  void get_is_legal_capture(std::vector<bool> &is_legal,
                            const std::array<int, 56> &circle) const;
  std::vector<bool> get_is_legal_action() const;
  bool no_leagal_actions() const;
  bool no_legal_capture(const std::array<int, 56> &circle) const;
  /** 0 -> B (black)
   * 1 -> W (white)
   * 2 -> E (empty) */
...
class SurakartaGame final : public Game {
 public:
...
namespace {
Registration<SurakartaGame> registration;
}

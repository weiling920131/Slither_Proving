#include "clap/game/slither5/slither5.h"
...
Slither5State::Slither5State(GamePtr game_ptr)
...
StatePtr Slither5State::clone() const {
  return std::make_unique<Slither5State>(*this);
}

void Slither5State::apply_action(const Action &action) {
...
std::vector<Action> Slither5State::legal_actions() const {
...
bool Slither5State::is_legal_action(const Action &action) const {
...
bool Slither5State::have_move(const Action &action) const {
...
bool Slither5State::have_win(const Action &action) const {
	if (action < 0 || action >= kNumOfGrids) return false;
	const Player player = board_[action];
	std::stack<int> pieces;
	pieces.push(int(action));
	int traversed[25] = {0}; //const
	traversed[action] = 1;
	int head = 0, tail = 0; 
	while(!pieces.empty()){
		int cur = pieces.top();
...
bool Slither5State::is_valid(const Action &action) const {
	if (action < 0 || action >= kNumOfGrids) return false;
	const Player player = board_[action];
	int top_check = 1, bottom_check = 1, left_check = 1, right_check = 1;
...
bool Slither5State::is_empty_valid(const Action &action) const {
	if (action < 0 || action >= kNumOfGrids) return false;
...
std::string Slither5State::to_string() const {
  std::stringstream ss;
...
bool Slither5State::is_terminal() const {
  //return (winner_ != -1) || (legal_actions().empty());
  return (winner_ != -1);
}

Player Slither5State::current_player() const {
  return (Player)((turn_ % 6) / 3);
}

std::vector<float> Slither5State::observation_tensor() const {
  std::vector<float> tensor;
  tensor.reserve(4 * 5 * 5); //8 * 6 * 6
  Player player = current_player();
...
std::vector<float> Slither5State::returns() const {
  if (winner_ == BLACK) {
    return {1.0, -1.0};
  }
  if (winner_ == WHITE) {
...
std::string Slither5State::serialize() const {
  std::stringstream ss;
...
std::string Slither5Game::name() const {
  return "slither5";
}
int Slither5Game::num_players() const { return 2; }
int Slither5Game::num_distinct_actions() const {
  return kPolicyDim;
}
std::vector<int> Slither5Game::observation_tensor_shape() const {
  return {4, kBoardSize, kBoardSize};
}

StatePtr Slither5Game::new_initial_state() const {
  return std::make_unique<Slither5State>(shared_from_this());
}

int Slither5Game::num_transformations() const { return 4; }

std::vector<float> Slither5Game::transform_observation(
...
std::vector<float> Slither5Game::transform_policy(
...
std::vector<float> Slither5Game::restore_policy(
    const std::vector<float> &policy, int type) const {
  std::vector<float> data(policy);
  if (type != 0) {
    for (int index = 0; index < kNumOfGrids; index++) {
...
int Slither5Game::transform_index(const int &index, const int type) const {
  if (type == 0) {
    return index;
  } else {
...
std::string Slither5Game::action_to_string(
...
std::vector<Action> Slither5Game::string_to_action(
...
StatePtr Slither5Game::deserialize_state(
    const std::string &str) const {
